...
c7_immune <- read.gmt("references/c7.immunesigdb.v7.4.symbols.gmt")
c7_immune_mat <-
  c7_immune %>%
  mutate(present = 1) %>%
  pivot_wider(
...
plier_c7_immune <- PLIER(vsd_exprs, c7_immune_mat)

pheatmap(
  mat = t(plier_c7_immune$B)[sample_disease_order,],
  scale="column",
  cluster_rows = FALSE,
...
plier_c7_immune_stats <-
  ...
  grouped_add_xy_positions(
    stats_tbl = .,
    data_tbl = plier_c7_immune_modules,
    group_var = module,
    compare_value = score
  )
...
plier_c7_immune_modules %>%
  ...
  stat_pvalue_manual(
    data = plier_c7_immune_stats,
    label =
      ifelse(
        test = ("p.adj.signif" %in% plier_c7_immune_stats),
        yes = "p.adj.signif",
        no = "p"
      ),
    hide.ns = TRUE,
...
LV7_data %>%
  ...
  stat_pvalue_manual(
    ...
    label =
      ifelse(
        test = ("p.adj.signif" %in% LV7_stats),
        yes = "p.adj.signif",
        no = "p"
      ),
    hide.ns = TRUE,
